...
esp_err_t module_load_from_flash(uint32_t flash_addr, module_handle_t *handle)
{
    ...
    // Read module header from flash
    const esp_partition_t *partition = esp_partition_find_first(
        ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_ANY, "modules");
    
    if (partition == NULL) {
        ESP_LOGE(TAG, "Modules partition not found");
        return ESP_ERR_NOT_FOUND;
    }

    esp_err_t ret = esp_partition_read(partition, flash_addr, 
                                       &mod->header, sizeof(module_header_t));
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to read module header: %s", esp_err_to_name(ret));
        return ret;
    }

    // Validate header
    ret = validate_module_header(&mod->header);
    if (ret != ESP_OK) {
        return ret;
    }

    // Allocate memory for code (must be executable)
    ...
    // Read code from flash
    ret = esp_partition_read(partition, flash_addr + sizeof(module_header_t),
                            mod->code_mem, mod->header.code_size);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to read module code: %s", esp_err_to_name(ret));
        free(mod->code_mem);
        return ret;
    }
    ...
    if (mod->header.data_size > 0) {
        ...
        if (mod->data_mem == NULL) {
            ESP_LOGE(TAG, "Failed to allocate %lu bytes for data", 
                     mod->header.data_size + mod->header.bss_size);
            free(mod->code_mem);
            return ESP_ERR_NO_MEM;
        }

        // Read data section
        ret = esp_partition_read(partition, 
                                flash_addr + sizeof(module_header_t) + mod->header.code_size,
                                mod->data_mem, mod->header.data_size);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to read module data: %s", esp_err_to_name(ret));
            free(mod->code_mem);
            free(mod->data_mem);
            return ret;
        }
    ...
    ESP_LOGI(TAG, "Module '%s' loaded successfully", mod->header.name);
    ESP_LOGI(TAG, "  Code at: %p", mod->code_mem);
    ESP_LOGI(TAG, "  Data at: %p", mod->data_mem);
    ESP_LOGI(TAG, "  Total memory: %zu bytes", g_loader.total_memory_allocated);
    ...
    if (mod->init_fn != NULL) {
        ...
        ret = mod->init_fn();
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Module init failed: %s", esp_err_to_name(ret));
        ...
        ESP_LOGI(TAG, "Module init successful");
    }
