...
OSD_Result_t Tab_OnButton(const Button_t Button, const ButtonState_t State, void *arg)
{
    ...
    // Fall-through to allow additonal A/B/Down/Up behavior to be handled by the current item
    switch (Button)
    {
        case kButton_A:
        case kButton_B:
        ...
        case kButton_Up:
            if (State == kButtonState_Pressed)
            {
                ...
                {
                    const OSD_Result_t eResult = pList->pCurrent->Widget.fnOnButton(Button, State, arg);

                    if (eResult != kOSD_Result_Ok)
                    {
                        ESP_LOGE(TAG, "%s OnButton call failed with %d", pList->pCurrent->Widget.Name, eResult);
                    }
                }
            }
            break;
